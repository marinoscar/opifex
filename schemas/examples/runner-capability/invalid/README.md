...
| `zero-concurrency.json`                         | A runner that will accept no work is not a runner. Zero here would route work to something that can never take it — a runner that cannot work _right now_ says so with `available: false` instead, which is a different fact |
| `unavailable-without-saying-why.json`           | `available: false` requires `unavailableReason`. "Unavailable" with no lead leaves the operator exactly as stuck as silence, and is where an afternoon goes                                                                  |
