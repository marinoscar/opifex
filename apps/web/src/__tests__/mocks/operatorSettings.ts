 ...
 *
 * The three secrets are deliberately in three different states, because those
 * are the three the section has to draw apart: `github.token` configured from
 * the environment (so there is nothing stored to clear),
 * `runners.claudeCodeLocal.oauthToken` configured from a stored row (so Clear
 * is live), and `supervisor.model.apiKey` not configured at all.
 *
...
export const OPERATOR_SETTINGS_FIXTURE: OperatorSetting[] = [
  ...
  },
  {
    ...
    help: 'The separately metered key the supervisor calls its model with.',
    type: 'string',
    reload: 'live',
    dangerous: true,
    source: 'default',
...
export function operatorSettingsFixture(
  ...
  return {
    ...
    overlay: {
      ...
      // Three entries above carry `source: 'database'`; the count the API
